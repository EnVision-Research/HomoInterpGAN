...
def warp_from_template(original, M, border_value=(0.5, 0.5, 0.5), image_dims=(400, 400)):
    return cv2.warpAffine(original, M[::-1], dsize=image_dims, flags=(cv2.INTER_AREA | cv2.WARP_INVERSE_MAP),
                          borderMode=cv2.BORDER_CONSTANT, borderValue=(0.0, 0.0, 0.0)).transpose(1, 0, 2)
...
def load_face_detector(predictor_path='util/shape_predictor_68_face_landmarks.dat'):
...
def compute_template(globspec='images/lfw_aegan/*/*.png', image_dims=[400, 400],
                     predictor_path='util/shape_predictor_68_face_landmarks.dat', center_crop=None, subsample=1):
